...
from util.loss import bpr_loss, l2_reg_loss, weight_bpr_loss, l2_reg_loss_weight
import time
...
import math
import os
from util.tool import  dataSave
...
class LightGCN():
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.args.batch_size)):
                ...
                if epoch == 5:
                    for i in range(len(user_idx)):
                        ...
                        loss = bpr_loss(temp_user_emb, temp_pos_item_emb, temp_neg_item_emb) + l2_reg_loss(self.args.reg, temp_user_emb,
                                                                                       temp_pos_item_emb)
                        if user_idx[i] not in self.data.loss:
                            self.data.loss[user_idx[i]] = loss.item()
                        else:
                            self.data.loss[user_idx[i]] += loss.item()
                        # self.data.loss[(user_idx[i], pos_idx[i])] = loss
    ...
    def train_anti(self, Epoch=0, optimizer=None, evalNum=5):
        self.bestPerformance=[]
        model = LGCN_Encoder(self.data, self.args.emb_size, self.args.n_layers).cuda()
        if optimizer is None: 
            self.optimizer = torch.optim.Adam(model.parameters(), lr=self.args.lRate)
        else:
            self.optimizer = optimizer
        ...
        for epoch in range(maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.args.batch_size)):
                ...
                rec_user_emb, rec_item_emb = model.weight_forward(self.weight_p)
                    
                user_emb, pos_item_emb, neg_item_emb = rec_user_emb[user_idx], rec_item_emb[pos_idx], rec_item_emb[
                    neg_idx]
            
                batch_loss = bpr_loss(user_emb, pos_item_emb, neg_item_emb) + l2_reg_loss(self.args.reg, user_emb, pos_item_emb)
    ...
    def save_anti(self):
        with torch.no_grad():
            self.best_user_emb, self.best_item_emb = self.model.weight_forward(self.weight_p)
...
class LGCN_Encoder(nn.Module):
    ...
    def weight_forward(self, weight_p):
        weight_p = TorchGraphInterface.convert_sparse_mat_to_tensor(weight_p).cuda()
        ego_embeddings = torch.cat([self.embedding_dict['user_emb'], self.embedding_dict['item_emb']], 0).cuda()
        all_embeddings = []  
        for k in range(self.layers):
            weight_norm_adj = self.sparse_norm_adj.multiply(weight_p)
            ego_embeddings = torch.sparse.mm(weight_norm_adj, ego_embeddings)
