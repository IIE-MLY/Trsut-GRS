...
class XSimGCL():
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.args.batch_size)):
                ...
                if epoch == 5:
                    for i in range(len(user_idx)):
                        ...
                        loss = bpr_loss(temp_user_emb, temp_pos_item_emb, temp_neg_item_emb) + l2_reg_loss(self.args.reg, temp_user_emb, temp_pos_item_emb) + self.cl_rate * self.cal_cl_loss([[user_idx[i]], [pos_idx[i]]], rec_user_emb,cl_user_emb,rec_item_emb,cl_item_emb)
        ...
        for epoch in range(maxEpoch):
            for n, batch in enumerate(next_batch_pairwise(self.data, self.args.batch_size)):
                ...
                user_emb, pos_item_emb, neg_item_emb = rec_user_emb[user_idx], rec_item_emb[pos_idx], rec_item_emb[
                    neg_idx]
                rec_loss = bpr_loss(user_emb, pos_item_emb, neg_item_emb)
                cl_loss = self.cl_rate * self.cal_cl_loss_weight([user_idx,pos_idx],rec_user_emb,cl_user_emb,rec_item_emb,cl_item_emb,self.weight_cl)
                batch_loss = rec_loss + l2_reg_loss(self.args.reg, user_emb, pos_item_emb) + cl_loss
                # Backward and optimize
                optimizer.zero_grad()
                batch_loss.backward()
    ...
    def cal_cl_loss_weight(self, idx, user_view1,user_view2,item_view1,item_view2, weight):
        u_idx = torch.unique(torch.Tensor(idx[0]).type(torch.long)).cuda()
        i_idx = torch.unique(torch.Tensor(idx[1]).type(torch.long)).cuda()
        u_idx_weight = u_idx.tolist()
        i_idx_weight = i_idx.tolist()
        user_cl_loss = InfoNCE_weight(user_view1[u_idx], user_view2[u_idx], self.temp, weight[u_idx_weight,:][:, u_idx_weight].cuda())
        item_cl_loss = InfoNCE_weight(item_view1[i_idx], item_view2[i_idx], self.temp, weight[i_idx_weight,:][:, i_idx_weight].cuda())
        return user_cl_loss + item_cl_loss
