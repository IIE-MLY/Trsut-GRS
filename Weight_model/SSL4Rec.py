...
class DNN_Encoder(nn.Module):
    ...
    def cal_cl_loss_weight(self, uidx, iidx, weight):
        user_view1, user_view_2, item_view1, item_view_2 = self.item_encoding(uidx, iidx)
        weight_item = weight[iidx,:][:, iidx].cuda()
        weight_user = weight[uidx,:][:, uidx].cuda()
        cl_loss = InfoNCE_weight(item_view1, item_view_2, self.tau, weight_item) + InfoNCE_weight(user_view1, user_view_2, self.tau, weight_user)
        return cl_loss
